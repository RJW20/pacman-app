...
class CharacterPosition:
    ...
    def __init__(self, position: tuple[int,int], offset: tuple[int,int], norm: int) -> None:
        self.x = PositionCoordinate(position[0], offset[0], norm)
        self.y = PositionCoordinate(position[1], offset[1], norm)

    def __add__(self, vector: Vector) -> CharacterPosition:
        diff_x = self.x + vector.d_x
        diff_y = self.y + vector.d_y
        return CharacterPosition((diff_x.absolute, diff_y.absolute), (diff_x.relative, diff_y.relative), self.x.norm)
